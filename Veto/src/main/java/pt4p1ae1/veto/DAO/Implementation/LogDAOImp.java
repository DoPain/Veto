package pt4p1ae1.veto.DAO.Implementation;

import com.google.inject.Inject;
import com.google.inject.Singleton;
import com.google.inject.persist.Transactional;
import pt4p1ae1.veto.DAO.LogDAO;
import pt4p1ae1.veto.Entity.Log;
import pt4p1ae1.veto.Entity.Log_;

import javax.inject.Provider;
import javax.persistence.EntityManager;
import java.util.List;

/**
 * Implementation of {@link pt4p1ae1.veto.DAO.LogDAO}.
 */
@Singleton
public class LogDAOImp implements LogDAO {

    /**
     * Entity Manager used to perform database operations.
     */
    @Inject
    private Provider<EntityManager> em;

    @Override
    @Transactional
    public void saveOrUpdate(Log log) {
        this.em.get().persist(log);
    }

    @Override
    @Transactional
    public List<Log> findAll() {
        StringBuilder query = new StringBuilder("from ");
        query.append(Log.class.getName());
        List<Log> logs =
                em.get().createQuery(query.toString()).getResultList();
        return logs;
    }

    @Override
    public Log findByName(String name) {
        StringBuilder query = new StringBuilder()
                .append("from ")
                .append(Log.class.getName())
                .append(" as room")
                .append(" where room.")
                .append(Log_.name.getName())
                .append(" = :name");

        List resultList = em.get()
                .createQuery(query.toString())
                .setParameter("name", name)
                .getResultList();

        if (resultList.size() > 0) {
            return (Log) resultList.get(0);
        }
        return null;
    }
}
