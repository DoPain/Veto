package pt4p1ae1.veto.DAO.Implementation;

import com.google.inject.Inject;
import com.google.inject.Singleton;
import com.google.inject.persist.Transactional;
import pt4p1ae1.veto.DAO.PersonneDAO;
import pt4p1ae1.veto.Entity.Personne;
import pt4p1ae1.veto.Entity.Personne_;

import javax.inject.Provider;
import javax.persistence.EntityManager;
import java.util.List;

/**
 * Implementation of {@link pt4p1ae1.veto.DAO.PersonneDAO}.
 */
@Singleton
public class PersonneDAOImp implements PersonneDAO {

    /**
     * Entity Manager used to perform database operations.
     */
    @Inject
    private Provider<EntityManager> em;

    @Override
    @Transactional
    public void saveOrUpdate(Personne personne) {
        this.em.get().persist(personne);
    }

    @Override
    @Transactional
    public List<Personne> findAll() {
        StringBuilder query = new StringBuilder("from ");
        query.append(Personne.class.getName());
        List<Personne> personnes =
                em.get().createQuery(query.toString()).getResultList();
        return personnes;
    }

    @Override
    public Personne findByName(String name) {
        StringBuilder query = new StringBuilder()
                .append("from ")
                .append(Personne.class.getName())
                .append(" as room")
                .append(" where room.")
                .append(Personne_.name.getName())
                .append(" = :name");

        List resultList = em.get()
                .createQuery(query.toString())
                .setParameter("name", name)
                .getResultList();

        if (resultList.size() > 0) {
            return (Personne) resultList.get(0);
        }
        return null;
    }
}
