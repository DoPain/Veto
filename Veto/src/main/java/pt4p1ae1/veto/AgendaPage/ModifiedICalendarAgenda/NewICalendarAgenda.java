package pt4p1ae1.veto.AgendaPage.ModifiedICalendarAgenda;
...
import jfxtras.icalendarfx.components.*;
...
import jfxtras.internal.scene.control.skin.agenda.icalendar.base24hour.*;
...
import pt4p1ae1.veto.AgendaPage.RendezVousEntityOservable;
...
/**
 * <p>The {@link NewICalendarAgenda} control is designed to take a {@link VCalendar VCALENDAR} object,
 * which is based on the iCalendar RFC 5545 standard, and renders it in {@link Agenda}, which is a calendar
 * display control. {@link NewICalendarAgenda} renders only the {@link VDisplayable displayable}
 ...
 *
 * <p>The {@link NewICalendarAgenda} control has a number of features, including:
 * <ul>
 ...
 *
 * <p>Next, the {@link VCalendar VCALENDAR} instance must be provided in the {@link NewICalendarAgenda} constructor
 * as shown below:.
 * <pre> {@code ICalendarAgenda iCalendarAgenda = new ICalendarAgenda(vCalendar);}</pre>
 * Nothing else special is required to instantiate {@link NewICalendarAgenda} if you use the default factories.</p>
 *
 * <p> A simple example to display a {@link NewICalendarAgenda} with an example {@link VEvent VEVENT} is below:
 *
 * <pre>
 * {@code
 ...
 * </p>
 *
 * @author David Bal modified by Bastien Vincent
 * modification : add getter to AppointmentVComponentMap
 * @see <a target="_blank" href="https://tools.ietf.org/html/rfc5545#section-3.8.2.2">iCalendar 5545 Specification</a>
 */
public class NewICalendarAgenda extends Agenda {
    ...
    public static final String DEFAULT_PRODUCT_IDENTIFIER = ("-//JFxtras//iCalendarAgenda " + NewICalendarAgenda.MY_VERSION + "//EN");
    ...
    public NewICalendarAgenda withOrganizer(Organizer organizer) {
        setOrganizer(organizer);
        return this;
    }

    public NewICalendarAgenda withOrganizer(String organizer) {
        setOrganizer(organizer);
        return this;
    }
     ...
     */
    public NewICalendarAgenda withUidGeneratorCallback(jfxtras.icalendarfx.utilities.Callback<Void, String> uidCallback) {
     ...
     */
    public VComponentFactory<Appointment> getVComponentFactory() {
        return vComponentFactory;
    }

    /**
     * Sets the value of the {@link VComponent} factory
     */
    public void setVComponentFactory(VComponentFactory<Appointment> vComponentFactory) {
     ...
     */
    public RecurrenceFactory<Appointment> getRecurrenceFactory() {
     ...
     */
    public void setRecurrenceFactory(RecurrenceFactory<Appointment> recurrenceFactory) {
    ...
    /*
    Just add a getter to simplify the code
     */
    public Map<Integer, VDisplayable<?>> getAppointmentVComponentMap() {
        return appointmentVComponentMap;
    }
     ...
     */
    public void setSelectedOneAppointmentCallback(Callback<Appointment, Void> c) {
        selectedOneAppointmentCallback = c;
    }
     ...
     * and clicking other appointments does not trigger this callback.
     */
    public Callback<Appointment, Void> getSelectedOneAppointmentCallback() {
        return selectedOneAppointmentCallback;
    }
     ...
     */
    public Callback<Appointment, ButtonData> getNewAppointmentDrawnCallback() {
        return newAppointmentDrawnCallback;
    }
     ...
     * added in Agenda.  This is done by clicking on the start time, dragging, and releasing on the end time.
     */
    public void setNewAppointmentDrawnCallback(Callback<Appointment, ButtonData> c) {
        newAppointmentDrawnCallback = c;
    }
     ...
     * CONSTRUCTOR
     */
    public NewICalendarAgenda() {
        this(new VCalendar()); // create empty VCalendar
    }

    public NewICalendarAgenda(VCalendar vCalendar) {
        super();
        ...
        setNewAppointmentCallback((LocalDateTimeRange dateTimeRange) ->
        {
            ...
            return new AppointmentImplTemporal()
                    .withStartTemporal(s)
                    .withEndTemporal(e)
...
//                    .withDescription("")
        ...
        Callback<Appointment, Void> appointmentChangedCallback = (Appointment appointment) ->
        {
            ...
            if (AgendaPage.vEventEntity.containsKey(UID)) {
                RendezVousEntity changedRDV = RendezVousEntityOservable.toEntity(vComponent);
                RendezVousEntity oldRDV = AgendaPage.vEventEntity.get(UID);
        ...
        {
            VEvent vComponent = (VEvent) appointmentVComponentMap.get(System.identityHashCode(appointment));
            if (vComponent == null) { // having no VComponent in map means the appointment is new and VComponent must be created
            ...
            popupScene.getStylesheets().addAll(getUserAgentStylesheet(), NewICalendarAgenda.ICALENDAR_STYLE_SHEET);
